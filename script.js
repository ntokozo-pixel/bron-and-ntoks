const STORAGE_KEYS={history:"ntokozo_vibe_history",memories:"ntokozo_memory_gallery",stories:"ntokozo_story_state",playlists:"ntokozo_custom_playlists",stats:"ntokozo_app_stats",specials:"ntokozo_special_flags"};
...
const introPhrases=[
"I hope this makes you smile a little \u2764\uFE0F",
"I made this because I wanted to ask you something softly \u2764\uFE0F",
"Take a breath and relax while this loads \u2764\uFE0F",
"This is for you \u2764\uFE0F",
...
function recordAnswer(option,buttonEl){const dayData=getCurrentDayData();const question=dayData.questions[appState.currentQuestionIndex];appState.answers.push({prompt:question.prompt,type:question.type,label:option.label,weight:option.weight});appState.noAnswerStreak=option.weight===1?appState.noAnswerStreak+1:0;const triggerJealous=appState.noAnswerStreak>=3;els.answers.querySelectorAll("button").forEach((button)=>{button.disabled=true;});if(buttonEl){buttonEl.classList.add("is-selected");}if(navigator.vibrate){navigator.vibrate(40);}window.setTimeout(()=>{appState.currentQuestionIndex+=1;if(appState.currentQuestionIndex>=dayData.questions.length){if(triggerJealous){appState.afterJealousTarget="analysis";appState.noAnswerStreak=0;if(navigator.vibrate){navigator.vibrate(90);}showScreen("jealousMode");return;}startAnalysis();return;}renderQuestion();if(triggerJealous){appState.afterJealousTarget="app";appState.noAnswerStreak=0;if(navigator.vibrate){navigator.vibrate(90);}showScreen("jealousMode");}},220);}
...
async function handleUpload(event,key){const[file]=event.target.files||[];if(!file){return;}const src=await fileToDataUrl(file);if(key===STORAGE_KEYS.memories){const existing=getStorageArray(key);existing.push({src,type:file.type||"image",source:"user"});setStorage(key,existing.slice(-24));renderMemoryGallery();}else{const stories=getStoryState();stories[appState.activeStoryOwner]={src,ts:Date.now(),type:file.type||"image"};setStorage(STORAGE_KEYS.stories,stories);renderStoryChips();openStory(appState.activeStoryOwner);}renderStats();event.target.value="";}
...
function updateReminderStatus(lastVisitBeforeLoad){const stats=getStats();if(!("Notification" in window)){els.reminderStatus.textContent="Browser notifications are limited here, but the cute reminder card is ready.";return;}if(lastVisitBeforeLoad&&lastVisitBeforeLoad!==getTodayKey()){els.reminderStatus.textContent=getRandomReminder();return;}els.reminderStatus.textContent=stats.reminderPermission==="granted"?"Daily reminder permission is on. Web apps still need browser support to alert in the background.":"Tap enable reminders if you want browser notifications when supported.";}
async function enableReminders(){if(!("Notification" in window)){els.reminderStatus.textContent="This browser does not support notifications.";return;}const permission=await Notification.requestPermission();const stats=getStats();stats.reminderPermission=permission;saveStats(stats);updateReminderStatus("");if(permission==="granted"){new Notification("Bronwyn reminder",{body:getRandomReminder()});}}
...
function stopAmbientAudio(){bgMusic.pause();if(els.secretAudio){els.secretAudio.pause();if(els.secretPlayBtn){els.secretPlayBtn.querySelector("i").className="fas fa-play";}if(els.secretLoveNote){els.secretLoveNote.classList.remove("active");}}}
