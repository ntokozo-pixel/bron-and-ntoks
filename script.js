...
function renderProfile(){const stats=getStats();const history=getHistory();const stories=Object.values(getStoryState()).filter(Boolean).length;const playlists=getStorageArray(STORAGE_KEYS.playlists).length;const minutes=Math.round((stats.secondsSpent||0)/60);const avg=history.length?Math.round(history.reduce((sum,item)=>sum+item.score,0)/history.length):0;const joined=stats.daysOpened&&stats.daysOpened.length?formatDate(`${stats.daysOpened[0]}T00:00:00`):"Today";const lastVisit=stats.lastVisitDate?formatDate(`${stats.lastVisitDate}T00:00:00`):"Today";if(els.profileSubtitle){els.profileSubtitle.textContent=`${history.length?`Average vibe ${avg}%`:"Start your first daily check-in."}`;}if(els.profileNotificationTag){els.profileNotificationTag.textContent=`Notifications ${Notification.permission==="granted"?"on":"off"}`;}if(els.profileStoryTag){els.profileStoryTag.textContent=`${stories} live stor${stories===1?"y":"ies"}`;}if(els.profileQuickStats){els.profileQuickStats.innerHTML="";[{label:"Visits",value:stats.visits||0},{label:"Minutes",value:minutes},{label:"Check-ins",value:stats.checkInsCompleted||0},{label:"Playlists",value:playlists}].forEach((item)=>{const card=document.createElement("article");card.className="profile-mini-stat";card.innerHTML=`<span>${item.label}</span><strong>${item.value}</strong>`;els.profileQuickStats.appendChild(card);});}createProfileRows([{label:"Member since",value:joined},{label:"Last active",value:lastVisit},{label:"Memories saved",value:getAllMemories().length},{label:"Live stories",value:stories||0},{label:"Average vibe",value:history.length?`${avg}%`:"-"}],els.profileDetailList);createProfileRows([{label:"Notifications",value:Notification.permission==="granted"?"Allowed":"Off"},{label:"Location card",value:appState.remoteLocation?"Live":"Waiting"},{label:"Custom playlists",value:playlists||0},{label:"Theme streak",value:`${calculateStreak(history)} days`}],els.profilePreferenceList);}
function buildComplaintLink(){const text=(els.complaintText?.value||"").trim()||"I have a complaint about the relationship/app.";return `https://wa.me/27608793239?text=${encodeURIComponent(text)}`;}
function openComplaintWhatsApp(){window.open(buildComplaintLink(),"_blank","noopener,noreferrer");}
function formatLocationTime(value){if(!value){return "Waiting for a live location.";}return `Updated ${new Date(value).toLocaleTimeString([], {hour:"2-digit",minute:"2-digit"})}`;}
function buildMapUrls(location){const lat=Number(location.latitude);const lng=Number(location.longitude);const offset=0.01;const embed=`https://www.openstreetmap.org/export/embed.html?bbox=${lng-offset}%2C${lat-offset}%2C${lng+offset}%2C${lat+offset}&layer=mapnik&marker=${lat}%2C${lng}`;const external=`https://maps.apple.com/?ll=${lat},${lng}&q=Ntokozo`;return{embed,external};}
function renderLocationCard(){if(!els.locationStatus||!els.boyfriendMap||!els.locationUpdated){return;}const location=appState.remoteLocation;if(!location||location.is_active===false){els.locationStatus.textContent="No live location yet.";els.locationUpdated.textContent=appState.locationSharing?"Sharing from Ntokozo's laptop." : "Waiting for a live location.";els.boyfriendMap.src="about:blank";return;}const urls=buildMapUrls(location);els.boyfriendMap.src=urls.embed;const accuracy=Math.round(location.accuracy||0);els.locationStatus.textContent=`Ntokozo is here. Accuracy ${accuracy||"-"}m.`;els.locationUpdated.textContent=formatLocationTime(location.updated_at);}
function renderDailyMotivation(){if(!els.motivationQuote||!els.motivationAuthor||!els.dailyMotivationCard){return;}const index=getDayOfYear()%MOTIVATION_QUOTES.length;const quote=MOTIVATION_QUOTES[index];const colors=MOTIVATION_THEMES[index%MOTIVATION_THEMES.length];els.motivationQuote.textContent=`"${quote.text}"`;els.motivationAuthor.textContent=`- ${quote.author}`;els.dailyMotivationCard.style.setProperty("--motivation-a",colors[0]);els.dailyMotivationCard.style.setProperty("--motivation-b",colors[1]);}
async function startLocationSharing(){if(!navigator.geolocation){if(els.locationStatus){els.locationStatus.textContent="This device cannot share location.";}return;}if(appState.locationWatchId!==null){appState.locationSharing=true;localStorage.setItem(LOCATION_SHARE_KEY,"true");renderLocationCard();return;}appState.locationSharing=true;localStorage.setItem(LOCATION_SHARE_KEY,"true");els.locationStatus.textContent="Getting your laptop location...";appState.locationWatchId=navigator.geolocation.watchPosition(async(position)=>{const payload={owner:"ntokozo",latitude:position.coords.latitude,longitude:position.coords.longitude,accuracy:Math.round(position.coords.accuracy||0),is_active:true,updated_at:new Date().toISOString()};appState.remoteLocation=payload;renderLocationCard();if(REMOTE_ENABLED){try{await saveRemoteLocation(payload);}catch(error){console.warn("Remote location update failed",error);}}},(error)=>{appState.locationSharing=false;localStorage.removeItem(LOCATION_SHARE_KEY);if(els.locationStatus){els.locationStatus.textContent=error&&error.message?error.message:"Location permission was not granted.";}},{enableHighAccuracy:true,maximumAge:10000,timeout:12000});}
